...
def __run_lookahead_analysis_bg(config_loc: Config, job_id: str):
    ...
    try:
        ...
        logger.error(f"Lookahead analysis encountered a configuration Error: {e}")
        job["status"] = "failed"
        job["error"] = str(e)
    except (Exception, OperationalException, DependencyException) as e:
        logger.exception(f"Lookahead analysis caused an error: {e}")
        job["status"] = "failed"
        job["error"] = str(e)
    finally:
...
@router.post("/lookahead_analysis", response_model=BgJobStarted)
...
@router.get("/lookahead_analysis/{jobid}", response_model=LookaheadAnalysisResponse)
def api_get_lookahead_analysis(jobid: str):
    if not (job := ApiBG.jobs.get(jobid)):
        raise HTTPException(status_code=404, detail="Job not found.")
    if job["category"] != "lookahead_analysis":

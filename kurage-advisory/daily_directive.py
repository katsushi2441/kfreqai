...
regime calls plus current price stats, and asks Claude to give a coarse
...
Uses the claude binary bundled with the VS Code Claude Code extension
(same mechanism as url2ai/src/oss_worker.py's claude_request()), not the
paid Anthropic API -- this is the user's existing OAuth subscription.
"""
import glob
import json
import os
...
def main():
    ...
    try:
        ...
        print(f"[daily_directive] claude call failed, defaulting to neutral: {exc}", flush=True)
        directive, note = "neutral", "claude呼び出し失敗のためneutral"

    entry = advisory_state.write_directive(directive, note, f"claude-{CLAUDE_MODEL}")

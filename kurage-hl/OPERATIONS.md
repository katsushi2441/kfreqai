...
## kcbrain ゲートは 40 銘柄を送っている（トークン予算と直結）

`market_gate()` は `assets[:40]` を kcbrain に送る（kcbrain のスキーマ上限も 40）。
opportunity-ranking は「全銘柄を省略せず1行ずつ返す」仕様なので、**送る銘柄数が
そのまま LLM の必要出力トークン数になる**。

2026-08-03 に kcbrain 側の `num_predict` が銘柄数によらず 2200 固定だったため、
40 銘柄で出力が切り詰められ JSON が壊れ、**6時間で 64%(65/102) が 502** だった。
ゲートは fail-open なので取引は止まらず、代わりに **AI 判断が効かないまま素の戦略で
動き続ける**という気づきにくい壊れ方をしていた。

kcbrain 側を `token_budget_for()`（`600 + 90*銘柄数`、下限2200・上限8192）に修正済み
（kcbrain `348c713`）。**ここの `[:40]` を増やすときは kcbrain の上限 8192 と
1リクエストの所要時間（40銘柄で実測 38〜70 秒）を必ず確認すること。**
`HL_CYCLE_SECONDS=300` を超えると 1 サイクルが破綻する。

なお kfreqai 側の同じ kcbrain 利用（`kurage-advisory/kcbrain_shadow.py`）は
`MAX_ASSETS = 12` なので影響を受けていなかった。**同じ API でも呼び出し元で
送信数が違う**ことを前提に切り分ける。

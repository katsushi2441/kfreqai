...
function renderKpi(d, spot) {
  ...
  // 初期資金(先物ウォレット2000 + 現物ペーパー初期)を基準に表示し、残高と累計損益の整合を見せる
  ...
  document.getElementById('kpi-grid').innerHTML =
    ...
    card('残高（推定）', usd(totalBal), '初期 ' + usd(initTotal) + '・先物 ' + usd(dash.account_value_usd) + ' ＋ 現物 ' + usd(spotBal) + '（kfreqaiのちょうど1/10スケール）', totalBal < initTotal ? 'down' : 'up') +
